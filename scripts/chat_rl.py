...
parser.add_argument(
    ...
    help="reward function used for GSM8K RL",
)
# Optimization
parser.add_argument(
    "--embedding-lr",
...
wandb_run = (
    DummyWandb()
    if use_dummy_wandb
    else wandb.init(project="nanochat-rl", entity=wandb_entity, name=args.run, config=user_config)
)
...
print0(f"Using reward system: {args.reward_system}")
...
def get_batch():
    ...
    for example_idx in itertools.cycle(rank_indices):
        ...
        for sample_tokens in generated_token_sequences:
            ...
            reward, _ = reward_fn(train_task, conversation, generated_text)
