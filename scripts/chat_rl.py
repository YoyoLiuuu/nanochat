...
parser.add_argument(
    ...
    type=str,
    default="baseline",
    choices=["baseline", "numeric_distance", "completion_brevity", "combined"],
...
wandb_run = (
    ...
    if use_dummy_wandb
        ...
        project=os.environ.get("WANDB_PROJECT", "nanochat-rl"),
        entity=os.environ.get("WANDB_ENTITY", "yoyoliuuu"),
        name=args.run,
        config=user_config,
    )
)
...
def get_batch():
    ...
    for example_idx in itertools.cycle(rank_indices):
        ...
        for sample_tokens in generated_token_sequences:
            ...
            reward, components = reward_fn(train_task, conversation, generated_text)
            rewards.append(reward)
            reward_components_all.append(components)
        ...
        if reward_components_all:
            keys = reward_components_all[0].keys()
            reward_component_means = {
                key: float(
                    sum(component[key] for component in reward_components_all)
                    / len(reward_components_all)
                )
                for key in keys
            }
        # yield inputs/targets as (B, T) of ids and rewards as (B,) of floats
        yield generated_token_sequences, inputs, targets, rewards, advantages, reward_component_means
...
for step in range(num_steps):
    ...
    if step % args.eval_every == 0:
        ...
        sample_table = wandb.Table(
            columns=["step", "question", "reference", "model_output", "is_correct"]
        )
    ...
    if reward_component_count > 0:
        reward_component_logs = {
            f"reward_comp/{key}": value / reward_component_count
